...
public class Swipe {
    ...
    public static void swipeUp() throws IOException, ParseException {
        ...
        if (getPlatformUnderTest().getPlatformName().equals("ios")) {
            ...
            new TouchAction(Drivers.getMobileDriver()).press(PointOption.point(width / 5, height - 2)).moveTo(PointOption.point(width / 5, -height - 1)).release().perform();
        } else {
            MyLogger.log.info("Swipe up using android Driver");
            int x = width - 10;
            double startY = height * 0.90;
    ...
    public static void swipeDown() {
        ...
        int height = screenDim.height;
        int width = screenDim.width;
        MyLogger.log.info("Swipe up using drivers androidDriver swipe command");
        int x = width - 10;
        double startY = height * 0.20;
